...
output = pypandoc.convert_file("test/test.docx", 'html')
...
def get_tags_between_elements(start_element="", end_element=""):
    json_arr = {}
    clear_h1 = start_element.get_text().replace("\r\n"," ")
    json_arr[f"{clear_h1}"] = []
    ...
    while current_element and current_element != end_element:
        ...
        only_text_element = current_element.get_text().replace("\r\n"," ").replace("\n"," ")

        if "title" in str(current_element):
            json_format = {"title":only_text_element.replace("title: ","")}
        elif "description" in str(current_element):
            json_format = {"description":only_text_element.replace("description: "," ")}
        else:
            json_format = {current_element.name:element}

        json_arr[f"{clear_h1}"].append(json_format)
...
for i in range (len(all_h1)-1):
    jsons.update(get_tags_between_elements(all_h1[i], all_h1[i+1]))
print(jsons)  
with open("test/text_test.json","w",encoding="UTF-8") as f:
